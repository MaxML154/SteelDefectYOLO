...
def create_summary_csv():
    ...
    # Step 1. Read every xml file from flat annotations folder
    ...
    # Step 3. Save
    output_dir = os.path.join(ROOT_FOLDER_PATH, OUTPUT_FOLDER)
    os.makedirs(output_dir, exist_ok=True)
    df.to_csv(os.path.join(output_dir, 'annotations.csv'), index=False, encoding='utf-8-sig')
...
def plot_class_per_image():
    ...
    output_dir = os.path.join(ROOT_FOLDER_PATH, OUTPUT_FOLDER)
    df = pd.read_csv(os.path.join(output_dir, 'annotations.csv'), encoding='utf-8-sig')
    ...
    plt.grid(True)
    output_dir = os.path.join(ROOT_FOLDER_PATH, OUTPUT_FOLDER)
    plt.savefig(os.path.join(output_dir, 'n_class_per_image.png'))
    plt.close()
...
def plot_class_count():
    ...
    series = df.value_counts(subset=['class_name'], sort=False)
    counts = series.to_numpy()
    classes = series.index.get_level_values('class_name').values

    x = np.arange(len(classes))
    fig, ax = plt.subplots(figsize=(8,5))
    rects = ax.bar(x, counts, color='steelblue')

    ax.set_ylabel('Count')
    ax.set_title('Box count by class')
    ax.set_xticks(x)
    ax.set_xticklabels(classes)
    autolabel(ax, rects)
    fig.tight_layout()
    plt.grid(True)
    plt.savefig(os.path.join(output_dir, 'n_class.png'))
    plt.close()
...
def plot_pairplot_asr():
    ...
    File: `{label}_correlation.png`
    File: `{label}_aspect_ratio.png`
    """
    ...
    for label in LABELS:
        ...
        # For each class, plot correlation of xc, yc, w, h
        ...
        plt.title(label)
        plt.savefig(os.path.join(output_dir, f'{label}_correlation.png'))
        plt.close()

        # For each class, plot aspect-ratio distribution
        class_df.hist(column=["aspect_ratio"],
                        bins=50, grid=True)
        plt.title(label)
        plt.xlabel('Aspect ratio')
        plt.ylabel('Count')
        plt.savefig(os.path.join(output_dir, f'{label}_aspect_ratio.png'))
        plt.close()


def plot_co_occurrence():
    """ Plot correlation of class. File: `co_occurrence.png`"""
    ...
    # Iterate each image, find different box name.
    for index, row in df.iterrows():
        ...
        # Put point on corresponding scatter plot
        ax[row_idx][col_idx].plot(row['xc'], row['yc'], 'o', markersize=2, markeredgecolor='b')

    plt.savefig(os.path.join(output_dir, 'co_occurrence.png'))
    plt.close()


def plot_anchor_shape():
    """ Plot the shape of each box. File: `{label}_anchor.png`"""
    ...
    for label in LABELS:
        ...
        # Iterate each image, shift the box to center of frame.
        ...
        plt.savefig(os.path.join(output_dir, f'{label}_anchor.png'))
        plt.close()


def plot_brightness_histogram():
    """ Plot the grayscale histogram. File: `{label}_brightness.png`"""
    ...
    for label in LABELS:
        ...
        for index, row in class_df.iterrows():
            # Read image from flat images folder
            img_fn = row['file_name'].replace('xml', 'jpg')
            fp = os.path.join(ROOT_FOLDER_PATH, IMG_FOLDER, img_fn)
        ...
        plt.grid()
        plt.savefig(os.path.join(output_dir, f'{label}_brightness.png'))
        plt.close()
...
def print_detailed_statistics():
    """ Print detailed statistics to console and save to text file. """
    output_dir = os.path.join(ROOT_FOLDER_PATH, OUTPUT_FOLDER)
    df = pd.read_csv(os.path.join(output_dir, 'annotations.csv'), encoding='utf-8-sig')
    ...
    line = f'总图像数: {df["file_name"].nunique()}'
    output_lines.append(line)
    print(line)

    line = f'平均每图框数: {len(df) / df["file_name"].nunique():.2f}'
    output_lines.append(line)
    print(line)
    ...
    for cls in LABELS:
        ...
        line = f'  数量: {len(cdf)} ({len(cdf)/len(df)*100:.1f}%)'
        output_lines.append(line)
        print(line)

        line = f'  宽度 - 均值:{cdf["w"].mean():.1f}, 中位数:{cdf["w"].median():.1f}, 标准差:{cdf["w"].std():.1f}'
        output_lines.append(line)
        print(line)

        line = f'  高度 - 均值:{cdf["h"].mean():.1f}, 中位数:{cdf["h"].median():.1f}, 标准差:{cdf["h"].std():.1f}'
        output_lines.append(line)
        print(line)

        line = f'  面积 - 均值:{area.mean():.0f}, 中位数:{area.median():.0f}, 标准差:{area.std():.0f}'
        output_lines.append(line)
        print(line)

        line = f'  长宽比 - 均值:{cdf["aspect_ratio"].mean():.2f}, 中位数:{cdf["aspect_ratio"].median():.2f}'
        output_lines.append(line)
        print(line)

        # 检测异常
        large_boxes = len(cdf[area > 30000])
        small_boxes = len(cdf[area < 1000])
        extreme_ratio = len(cdf[(cdf['aspect_ratio'] > 5) | (cdf['aspect_ratio'] < 0.2)])

        if large_boxes > 0:
            line = f'  ⚠ 超大框 (>30k像素): {large_boxes} ({large_boxes/len(cdf)*100:.1f}%)'
            output_lines.append(line)
            print(line)
        if small_boxes > 0:
            line = f'  ⚠ 超小框 (<1k像素): {small_boxes} ({small_boxes/len(cdf)*100:.1f}%)'
            output_lines.append(line)
            print(line)
        if extreme_ratio > 0:
            line = f'  ⚠ 极端长宽比 (>5或<0.2): {extreme_ratio} ({extreme_ratio/len(cdf)*100:.1f}%)'
            output_lines.append(line)
            print(line)
    ...
    # Save to file
    with open(os.path.join(output_dir, 'statistics_summary.txt'), 'w', encoding='utf-8') as f:
        f.write('\n'.join(output_lines))


if __name__ == '__main__':
    print("正在生成NEU-DET标注统计分析...")
    print(f"输出目录: {os.path.join(ROOT_FOLDER_PATH, OUTPUT_FOLDER)}")
